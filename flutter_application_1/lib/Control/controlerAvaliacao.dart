...
Future<int> addPrestador ( Comment avaliar) async{
...
//Register
return await db.insert('Comment',avaliar.toMap());
...
Future<int> editPrestador (Comment avaliar) async{

//criar variável do usuário no banco de dados para acessar o método.
final db = await bancodados.database;
//atualizar os dados no banco de dados.
return await db.update('Comment', avaliar.toMap(),where: 'id=?',whereArgs: [avaliar.idComment] );

}//fim do método editRegistro

//Método de listar Registro
Future<List<Comment>> getRegister(dynamic listadeAvaliacao) async{
  ...
  return List.generate(listadeAvaliacao.length, (int i){
    return Comment.fromMap(listadeAvaliacao[i]);
  });
